...
\noindent
I owned \textbf{RT model M2 (decision uncertainty, $\mu=a-b|2P-1|$)}: its specification, psychological motivation, fitting and evaluation. I also built the \textbf{inference machinery} (MAP objective, priors and their justification, the L-BFGS-B optimiser with multiple restarts, the MLE cross-check and the likelihood-curvature identifiability diagnostic) and the \textbf{test--retest reliability analysis} (Pearson/Spearman/ICC, bootstrap CIs, paired CIs on reliability differences).
...
\noindent
\textbf{Name and signature:} Saliq Neyaz\quad\rule{5cm}{0.4pt}
\hfill
